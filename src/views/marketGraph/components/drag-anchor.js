/*
 * @Author: Jelly
 * @Date: 2022-09-01 10:11:38
 * @LastEditTime: 2022-09-21 09:52:20
 * @LastEditors: Jelly
 * @Description: 注册锚点拖拽行为
 */
...
let disabled = window.location.hash.split('sendStatus=')[1] != 1;
export default registerBehavior => {
  registerBehavior('drag-anchor', {
    ...
    // 开始拖拽
    onDragStart(e) {
      if (e.target.cfg.name === 'anchor' && !disabled) {
        ...
        const line = group.addShape('path', {
          attrs: {
            stroke: '#1890FF',
            lineDash: [5, 3],
            path: [['M', bBox.width * (p[0] - 0.5), bBox.height * (p[1] - 0.5)]],
          },
          name: 'dashed-path',
        });
    ...
    onDrop(e) {
      if (e.target.cfg.name === 'anchor' && !disabled) {
        ...
        const { anchorIndex } = e.target.cfg;
        // 所有节点只有一条入边
        if (!node.getInEdges().length && this.sourceId) {
          // 发生连线
